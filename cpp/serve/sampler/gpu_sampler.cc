...
class GPUSampler : public SamplerObj {
 public:
  explicit GPUSampler(int max_num_sample, int vocab_size, FunctionTable* ft, DLDevice device,
    ...
    top_prob_offsets_host_ = NDArray::Empty({max_num_sample * 5}, dtype_i32_, preferred_host_device);
    ...
    top_prob_indices_host_ = NDArray::Empty({max_num_sample * 5}, dtype_i32_, preferred_host_device);
 ...
 private:
  ...
  NDArray GenerateUniformSamples(const std::vector<RandomGenerator*>& rngs,
    ...
    NDArray uniform_samples_device = uniform_samples_device_.CreateView({total_samples}, dtype_f32_);
    CopyArray(/*src=*/uniform_samples_host, /*dst=*/uniform_samples_device, copy_stream_);
    return uniform_samples_device;
  }
  ...
  std::vector<NDArray> SampleOnGPU(NDArray probs_on_device, NDArray uniform_samples_device,
    ...
    Array<NDArray> argsort_results = gpu_argsort_probs_func_(probs_on_device).cast<Array<NDArray>>();
    ...
    if (need_prob_values) {
      ...
      NDArray top_prob_offsets_host = top_prob_offsets_host_.CreateView({num_top_probs}, dtype_i32_);
